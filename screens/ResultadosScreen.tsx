...
import { fetchAllFixtures, fetchLiveFixtures, isLive, isFinished } from '../services/api';
...
export default function ResultadosScreen() {
  ...
  const { data: allFixtures, isLoading, error, isFetching } = useQuery({
    queryKey: ['fixtures'],
  ...
  // Notificar eventos cuando hay partidos en vivo
  useEffect(() => {
    if (liveFixtures && liveFixtures.length > 0) {
      checkAndNotify(liveFixtures);
    }
  }, [liveFixtures]);

  // Merge datos en vivo con la lista completa
  const merged = useMemo<Fixture[]>(() => {
    if (!allFixtures) return [];
    if (!liveFixtures?.length) return allFixtures;

    const liveMap = new Map(liveFixtures.map((f) => [f.fixture.id, f]));
    return allFixtures.map((f) => liveMap.get(f.fixture.id) ?? f);
  }, [allFixtures, liveFixtures]);

  const sections = useMemo<Section[]>(() => {
    const live = merged.filter((f) => isLive(f.fixture.status.short));
    ...
    if (filter === 'todos' || filter === 'finalizados') {
      ...
      for (const f of finished) {
        const key = new Date(f.fixture.date).toLocaleDateString('es-MX', {
    ...
    if (filter === 'vivo' && live.length === 0) {
      result.push({ title: 'SIN PARTIDOS EN VIVO', data: [] });
    }
  ...
  const filters: { key: Filter; label: string }[] = [
    ...
    { key: 'vivo', label: `En vivo${liveCount > 0 ? ` (${liveCount})` : ''}` },
  ...
  return (
    <View style={styles.container}>
          ...
          refreshControl={
            <RefreshControl
              ...
              onRefresh={() => {
                ...
                queryClient.invalidateQueries({ queryKey: ['fixtures-live'] });
              }}
      ...
      {!isLoading && !error && sections.every((s) => s.data.length === 0) && (
        <View style={styles.center}>
